...
lasso_spec <- logistic_reg(penalty = 0.01, 
                           mixture = 1) |> 
...
lasso_rs_predictions |> 
  ...
  labs(color = "Resamples",
       title = "ROC for Climbing Incidents Reports ")

```

```{r lasso-conf-mat}
confusion_matrix <- conf_mat_resampled(lasso_rs, tidy = F) |> 
  autoplot(type = "heatmap")
```
...
tune_spec <- logistic_reg(penalty = tune(), mixture = 1) |> #0 = ridge, 1= lasso
  set_mode("classification") |> 
  set_engine("glmnet")
...
autoplot(tune_rs) + 
  labs(title = "Lasso Performance across regularitazion penalties")
...
# within one standard error provided better results and had a little bit more penalization
...
# features are associated with words and can look at the top words for non-fatal

fitted_lasso |> 
  extract_fit_parsnip() |> # extracting model fit, it is pulling out the stuff from the model object but it is not clear to see what it is doing; extracting info from the model fit to make it accessible to us
  tidy() |> # putting it with the tidy fit
  arrange(-estimate) # arranging by estimate in descending order
...
fitted_lasso |> 
  extract_fit_parsnip() |> # extracting model fit
  tidy() |> # putting it with the tidy fit
  arrange(estimate) # arranging by estimate in descending order
